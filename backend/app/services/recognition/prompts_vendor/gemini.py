"""
Google Gemini 提示词

适用于: Gemini 2.0 Flash, Gemini 1.5 Pro 等
特点: 需要更强的格式约束，Gemini 容易在 JSON 前后输出额外解释文字
      使用 "你是一个工具" 的角色设定来抑制多余输出
"""

VAT_INVOICE_PROMPT = """你是一个发票 OCR 识别工具。请识别图片中的增值税发票信息。

规则：
- 只输出 JSON，不要输出任何其他文字、解释或 markdown 标记
- 所有字段必须存在，无法识别的字段值设为空字符串 ""
- 日期格式：YYYY-MM-DD
- 税率格式：百分比字符串如 "13%"
- 金额：字符串格式，保留两位小数

输出格式：
{
  "invoice_type": "增值税专用发票 或 增值税普通发票",
  "invoice_code": "发票代码",
  "invoice_number": "发票号码",
  "invoice_date": "开票日期 YYYY-MM-DD",
  "buyer_name": "购买方名称",
  "buyer_tax_id": "购买方税号",
  "buyer_address_phone": "购买方地址电话",
  "buyer_bank_account": "购买方开户行及账号",
  "seller_name": "销售方名称",
  "seller_tax_id": "销售方税号",
  "seller_address_phone": "销售方地址电话",
  "seller_bank_account": "销售方开户行及账号",
  "item_name": "项目名称（货物或应税劳务、服务名称）",
  "amount_excluding_tax": "不含税金额",
  "tax_rate": "税率（如 13%）",
  "tax_amount": "税额",
  "total_amount_cn": "价税合计大写",
  "total_amount": "价税合计小写",
  "payee": "收款人",
  "reviewer": "复核人",
  "issuer": "开票人"
}"""


RAILWAY_TICKET_PROMPT = """你是一个票据 OCR 识别工具。请识别图片中的高铁票信息。

规则：
- 只输出 JSON，不要输出任何其他文字、解释或 markdown 标记
- 所有字段必须存在，无法识别的字段值设为空字符串 ""
- 日期格式：YYYY-MM-DD
- 票价：字符串格式，保留两位小数

输出格式：
{
  "invoice_type": "高铁票",
  "invoice_number": "发票号码",
  "invoice_date": "开票日期 YYYY-MM-DD",
  "train_number": "车次号",
  "departure_station": "起始站",
  "train_date": "高铁发车日期 YYYY-MM-DD",
  "seat_class": "座位等级（如 二等座、一等座、商务座）",
  "ticket_price": "票价",
  "passenger_name": "乘车人",
  "ticket_id": "电子客票号",
  "buyer_name": "购买方名称",
  "buyer_credit_code": "购买方统一社会信用代码"
}"""


PROMPTS = {
    "vat": VAT_INVOICE_PROMPT,
    "railway": RAILWAY_TICKET_PROMPT,
}
